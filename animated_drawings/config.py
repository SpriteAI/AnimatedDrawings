...
class Config:
    def __init__(self, user_mvc_cfg_fn: str, mode="mesh") -> None:
        # get the base mvc config
        with open(resource_filename(__name__, "mvc_base_cfg.yaml"), "r") as f:
            base_cfg = defaultdict(
                dict, yaml.load(f, Loader=yaml.FullLoader) or {}
        ...
        self.scene: SceneConfig = SceneConfig({**base_cfg["scene"], **user_cfg["scene"]}, mode=mode)
        ...
        if self.controller.mode == "video_render":
            try:
                assert (
                    self.controller.output_video_path is not None
                ), "output_video_path must be set when using video_render controller"
            except AssertionError as e:
                msg = f"Config error: {e}"
                logging.critical(msg)
                assert False, msg
        ...
        if (
            self.controller.mode == "video_render"
            and self.controller.output_video_path is not None
            and self.controller.output_video_path.endswith(".mp4")
        ):
            try:
                assert (
                    self.controller.output_video_codec is not None
                ), "output_video_codec must be set when using video_render controller"
            except AssertionError as e:
                msg = f"Config error: {e}"
                logging.critical(msg)
                assert False, msg
...
class SceneConfig:
    def __init__(self, scene_cfg: dict, mode="mesh") -> None:
        ...
        for each in scene_cfg["ANIMATED_CHARACTERS"]:
            ...
            self.animated_characters.append(
                (CharacterConfig(char_cfg_fn, mode=mode), RetargetConfig(retarget_cfg_fn), MotionConfig(motion_cfg_fn))
            )
    ...
    def __init__(self, controller_cfg: dict) -> None:
        ...
        try:
            ...
            if isinstance(self.output_video_path, str):
                assert Path(self.output_video_path).suffix in (
                    ".gif",
                    ".mp4",
                ), "output video extension not .gif or .mp4 "
        except (AssertionError, ValueError) as e:
...
class CharacterConfig:
    ...
    def __init__(self, char_cfg_fn: str, mode: str = "mesh") -> None:  # noqa: C901
        ...
        try:
            ...
            for joint in char_cfg["skeleton"]:
                # For mesh deformation, don't care about eyes and ears
                if mode == "mesh" and joint["name"] in ["right_eye", "left_eye", "right_ear", "left_ear"]:
                    continue
        ...
        try:
            ...
            for joint in self.skeleton:
                assert (
                    isinstance(joint["parent"], NoneType) or joint["parent"] in names
                ), f"joint.parent not None and not valid joint name: {joint}"
        except AssertionError as e:
            msg = f"Error in character skeleton: {e}"
            logging.critical(msg)
            assert False, msg
    ...
    def validate_bvh(self, bvh_joint_names: List[str]) -> None:
        ...
        try:
            for prox_joint_name, dist_joint_name in self.forward_perp_joint_vectors:
                assert (
                    prox_joint_name in bvh_joint_names
                ), f"invalid prox_joint name in motion_cfg.forward_perp_joint_vectors: {prox_joint_name}"
                assert (
                    dist_joint_name in bvh_joint_names
                ), f"invalid dist_joint name in motion_cfg.forward_perp_joint_vectors: {dist_joint_name}"
        except (AssertionError, ValueError) as e:
...
class RetargetConfig:
    ...
    def __init__(self, retarget_cfg_fn: str) -> None:  # noqa: C901
        ...
        try:
            ...
            for group in self.bvh_projection_bodypart_groups:
                assert group["method"] in [
                    "pca",
                    "sagittal",
                    "frontal",
                ], 'group method must be "pca", "sagittal", or "frontal"'
        except (AssertionError, ValueError) as e:
        ...
        try:
            ...
            assert isinstance(
                self.char_bvh_root_offset["bvh_projection_bodypart_group_for_offset"], str
            ), "bvh_projection_bodypart_group_for_offset must be str"
        except (AssertionError, ValueError) as e:
    ...
    def validate_char_and_bvh_joint_names(
        self, char_joint_names: List[str], bvh_joint_names: List[str]
    ) -> None:  # noqa: C901
        ...
        try:
            ...
            group_names = [group["name"] for group in self.bvh_projection_bodypart_groups]
            assert (
                self.char_bvh_root_offset["bvh_projection_bodypart_group_for_offset"] in group_names
            ), "invalid bvh_projection_bodypart_group_for_offset"
        ...
        try:
            for check in self.char_runtime_checks:
                if check[0] == "above":
                    ...
                    assert (
                        target_joint_name in char_joint_names
                    ), f"above test target_joint_name invalid {target_joint_name}"
